...
- MVP 第一版通过 `run_no` 中的栏目后缀识别同栏目运行中任务；未来多进程或云端部署时升级为数据库任务锁。
- running 任务异常中断阈值通过 `APP_RUNNING_RUN_TIMEOUT_MINUTES` 配置，默认 360 分钟。
...
- 后台提供“测试抓取”按钮。
- 测试抓取默认不入库。
- MVP 第一版先展示来源栏目、抓取策略、解析记录数量、样例标题和失败原因。
- 轻量验证复用栏目抓取策略，支持 `http_static`、`http_with_headers`、`http_with_retry`、`json_api` 和 `browser_rendered`。
- `browser_rendered` 测试抓取需要本机已安装 Playwright 与 Chromium；未安装时展示明确安装提示。
- 后续增强版可展示列表页请求状态、前几条标题、日期、详情地址、正文片段、附件名称和附件地址。
- 展示失败原因和错误堆栈摘要。
...
- `configs/sites.yaml` 已接入 Excel 中可直接监测的 14 个启用栏目。
- 已覆盖住房和城乡建设部政策发布、行政规范性文件库、建设工程企业资质行政审批专栏多个子栏目。
- 已覆盖陕西省住房和城乡建设厅公告公示、省厅文件、陕西建筑施工公告 JSON 接口、陕西资质查询 JSON 接口和陕西资质增项公告查询 JSON 接口。
- 陕西资质增项公告查询页面已通过其前端实际调用的公开 JSON 接口接入，避免 V1 依赖 Playwright 渲染。
